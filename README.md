...
![DynamicEntropySearch Flow](Workflow_Figure_1.png)
...
In order to perform spectra comparison more efficient, spectral data should be loaded into a reference library in the form of index.
...
entropy_search.write()
```
**example_dynamic_3.py** can be an instruction.
...
```
You can run **example_dynamic_1.py** to get the same results.
...
You can use the `clean_spectrum()` function in `ms_entropy` to clean the query spectrum and then use individual search functions to search the library. **example_dynamic_3.py** can be an instruction.
...
```
You can run **example_dynamic_2.py** to get the same result.
...
The following search functions all need external clean step before searching. See [Search with external clean function](#search-with-external-clean-function). 
...
Another example **example_repository.py** can be found in the root directory of **DynamicEntropySearch** project and can serve as an instruction.  

Its result:
```
Top match spectrum data: {'precursor_mz': 38.96260452270508, 'charge': 1, 'rt': 118.33699798583984, 'peaks': array([[25.055452,  1.      ]], dtype=float32), 'file_name': 'metabolomics_workbench/ST002301/Plasma32-02_38_1_2339.mzML.gz', 'scan': np.uint64(1632), 'similarity': np.float32(0.9055619)}
```
