...
def _parse_information(line: bytes, spec: dict):
    ...
    :return: The entry of the added information.[str, str]
    """
    if line:
        lines = line.split(":")
        if len(lines) > 2:
...
def read_one_spectrum(filename_input: str, include_raw=0, parse_mol=False, parse_comments=False, **kwargs):
    """
    ...
    _scan_number = 1
    spectrum_info = {"ms_level": 2, "_scan_number": _scan_number, "peaks": [], "file": str(filename_input).split("/")[-1]}
    ...
    for line in fi:
        ...
        if is_spec_end:
            ...
            if spectrum_info.get("precursormz"):
                spectrum_info["precursor_mz"]=spectrum_info.pop("precursormz")
    ...
    if "comments" in spectrum_info:
        if isinstance(spectrum_info["comments"], list):
            for name, value in spectrum_info["comments"]:
                try:
                    if name.lower() == "smiles" or name.lower() == "computed smiles":
                    ...
                    pass
                try:
                    if name.lower() == "inchikey" or name.lower == "computed inchikey":
                        if value in mol_cache:
                            return mol_cache[value]
            ...
            mol_str_list = [[x] for x in filter(lambda x: x is not None, mol_str_list) if not isinstance(x, list)]
