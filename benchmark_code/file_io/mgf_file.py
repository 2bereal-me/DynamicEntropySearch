...
def read_one_spectrum(filename_input: str, include_raw=0, **kwargs):
    fi = open(filename_input, "rt")
    raw = []
    scan_number = 1
    spectrum_info = {"_scan_number": scan_number, "ms_level": 2, "peaks": [], "file": str(filename_input).split("/")[-1]}
    is_spectrum_start = 0
...
def write_spectrum(spec, fileout):
    ...
    if "precursor_charge" in spec:
        out_str.append("CHARGE=" + str(abs(spec["precursor_charge"])) + ("+" if spec["precursor_charge"] >= 0 else "-"))
...
def _write_spec(spec_info, spec_data, fileout):
    ...
    if "precursor_charge" in spec_info:
        out_str.append("CHARGE=" + str(abs(spec_info["precursor_charge"])) + ("+" if spec_info["precursor_charge"] >= 0 else "-"))
